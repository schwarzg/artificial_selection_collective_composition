...
cx3=plt.axes((0.15,0.05,0.33,0.33))
################################
...
for i in range(ncycle):
    ...
    #draw    
    #mean value
    #make total time table
    tstamp_all=np.sort(np.unique(np.concatenate(tsel)))
    cfs_all=np.zeros((ncomm,len(tstamp_all)))
    for j in range(0,ncomm):
        cfs=cf(wsel[j],msel[j],r=r,s=s)
        ind=0
        for k,t in enumerate(tsel[j]):
            ind2=np.argwhere(tstamp_all == t)[0][0]
            cfs_all[j,ind:ind2]=cfs[k]
            #print(t,'from',ind,'to',ind2,'put',cfs[k])
            ind=ind2
    ...
    mean_cfs=np.mean(cfs_all,axis=0)
    if i!=0:
        cx3.vlines(tstamp_all[0],mean_cfs[0],savelast,colors='red')
    cx3.plot(tstamp_all,mean_cfs,c='black')
    savelast=mean_cfs[-1]
    #save last
...
cx3.set_ylabel(r'F frequency $f$')
cx3.set_xlabel(r'Time $t$')
