...
def violinplot_from_histogram(ax,hists,bins,positions=None,showmeans=True,width=0.5,side='right',yoffset=None,marker='o',showmedians=False,medianmarker='o',**kwargs):
    ...
    for ind,dats in enumerate(hists):
        ...
        if side=='right' or side=='both':
            ...
            ax.fill_betweenx(x[ind]-yoffset[ind],positions[ind],positions[ind]+0.5*dats/maxval*width,**kwargs)
        if side=='left' or side=='both':
            #ax.fill_between(positions[ind]-0.5*dats/maxval*width,0,x[ind]-yoffset[ind],**kwargs)
            ax.fill_betweenx(x[ind]-yoffset[ind],positions[ind]-0.5*dats/maxval*width,positions[ind],**kwargs)
        if showmeans==True:
        ...
        if showmedians==True:
            cdf=np.cumsum(dats*dx[ind])
            medianval=quantile(0.5,bins[ind],dats,x0=np.min(bins[ind]))
            #print(meanval)
            medians.append(medianval)
            ax.scatter(positions[ind],medianval-yoffset[ind],**kwargs,marker=medianmarker)
    
    if showmeans and showmedians:
        return ax,means, medians
